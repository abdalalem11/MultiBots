...
DEFAULTS: Dict[str, Any] = {
    ...
    "restart_delay_base": float(os.environ.get("MB_RESTART_DELAY_BASE", "2")),
    "restart_delay_max": float(os.environ.get("MB_RESTART_DELAY_MAX", "120")),
...
class ConfigLoader:
    ...
    def load(self, path: Optional[str] = None) -> List[BotConfig]:
        ...
        if isinstance(bots_data, dict):
            ...
            raise ConfigError("config.json must contain a list of bots or a bots object")
...
class BotSupervisor:
    ...
    def download_source(self) -> None:
        target = Path(self.config.resolve_cwd(self.settings["bots_dir"]))
    ...
    def start(self) -> None:
        cwd = self.config.resolve_cwd(self.settings["bots_dir"])
        runfile = self.config.resolve_runfile(self.settings["bots_dir"])
        if not os.path.exists(runfile):
            raise FileNotFoundError(f"Run file not found: {runfile}")
        env = os.environ.copy()
        env.update(self.config.env)
        self.process = subprocess.Popen(
            [self.config.python, runfile, *self.config.args],
            cwd=cwd,
            env=env,
        )
    ...
    def stop(self) -> None:
        if self.process and self.process.poll() is None:
            ...
            try:
                self.process.wait(timeout=self.settings["shutdown_timeout"])
            ...
            for supervisor in self.supervisors:
                ...
                if proc and proc.poll() is not None:
                    ...
                    if supervisor.restarts < limit:
                        ...
                        self.logger.warning("Restarting %s", supervisor.config.name)
                        time.sleep(float(delay))
                        supervisor.start()
                    else:
                        self.logger.error("Restart limit reached for %s", supervisor.config.name)
            time.sleep(self.settings["watchdog_interval"])
